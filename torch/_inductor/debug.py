...
def create_kernel_information_json() -> dict[str, dict[str, Any]]:
    ...
    try:
        ...
        for kernel_key in all_kernels:
            ...
            kernel_name = kernel_key.rsplit(":", 1)[0]
            is_extern = kernel_name.startswith(("aoti_torch_", "extern_kernels."))
            ...
            elif is_extern and len(pre_grad_list) == 1:
...
def set_kernel_post_grad_provenance_tracing(
    ...
    try:
        ...
        if is_extern:
            ...
            try:
                from .ir import Buffer, ReinterpretView

                input_shapes: list[list[int | str]] = []
                input_dtypes: list[str] = []
                for inp in node_schedule.inputs:
                    if isinstance(inp, (Buffer, ReinterpretView)):
                        shape: list[int | str] = []
                        for dim in inp.get_size():
                            try:
                                shape.append(int(dim))
                            except (TypeError, ValueError):
                                shape.append(str(dim))
                        input_shapes.append(shape)
                        input_dtypes.append(str(inp.get_dtype()))
                output_shape: list[int | str] = []
                for dim in node_schedule.get_size():
                    try:
                        output_shape.append(int(dim))
                    except (TypeError, ValueError):
                        output_shape.append(str(dim))
                _inductor_kernel_shapes[kernel_name] = {
                    "input_shapes": input_shapes,
                    "input_dtypes": input_dtypes,
                    "output_shape": output_shape,
                    "output_dtype": str(node_schedule.get_dtype()),
                }
            except Exception as e:
